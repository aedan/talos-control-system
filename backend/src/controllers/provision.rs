...
fn generate_talos_secrets(
    ...
    // Use the standard Talos installer image. The v1.13.7 initramfs already
    // ships bnx2x.ko (Broadcom 10G) and util-linux disk tools, so no custom
    // factory schematic is required. The PXE installer is the same version, so
    // it reuses the locally-loaded image without a registry pull.
    let install_image = format!("ghcr.io/siderolabs/installer:{}", talos_version);
