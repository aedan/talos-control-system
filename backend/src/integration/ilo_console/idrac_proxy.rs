...
fn neutralize_idrac_framebust(text: &str) -> String {
    let t = text
        ...
        .replace("if (top !== self)", "if (false)")
        .replace("if(top!=self)", "if(false)");
    // sysSummary.html only calls progressBar.hide() when isCustomGui==1, so
    // the overlay spinner stays up in the TCS iframe even after
    // sysSummaryData.html has loaded.
    let t = t.replace(
        "if(top.isCustomGui == 1){\nprogressBar.hide();\n}",
        "progressBar.hide(); var _ps=document.getElementById(\"progressScreen\"); if(_ps)_ps.style.display=\"none\";",
    );
...
mod tests {
    ...
    fn syssummary_hides_progress_for_stock_gui() {
        ...
        assert!(out.contains("progressScreen"), "{out}");
        assert!(!out.contains("if(tcsTop.isCustomGui == 1){\nprogressBar.hide();"));
    }
